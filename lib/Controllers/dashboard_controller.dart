...
class DashboardController extends GetxController {
    ...
    try {
      server.postRequestWithToken(endPoint: APIList.deliverymanChangeStatus, body: jsonString).then((response) {
        ...
        if (response != null && response.statusCode == 200) {
          ...
          dashboardLoader.value = false;
          update();
          Get.rawSnackbar(title: 'Change Error', message: 'Enter a valid input', backgroundColor: CupertinoColors.destructiveRed.withOpacity(.9), margin: const EdgeInsets.only(bottom: 20, left: 20, right: 20));
        }
      });
    } catch (e) {
      Get.log(e.toString());
  ...
  changePickUpStatus(context, parcelId) {
    dashboardLoader.value = true;
    update();
    final queryParameters = {'parcel_id': parcelId, 'status_action': collectStatusID, 'cash_collection': cashController.text, 'note': noteController.text};
