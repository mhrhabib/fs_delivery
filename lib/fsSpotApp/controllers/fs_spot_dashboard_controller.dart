...
class FsSpotDashboardController extends GetxController {
  ...
  String returnDeliveryStatusID = '39';
  ...
  changeStatusWithOutSignature(context, statusID, parcelId) {
    dashboardLoader.value = true;

    update();
    Map<String, String> body = {'parcel_id': parcelId, 'status_action': collectByDriverStatusID!, 'note': noteController.text};
  ...
  // spotManagerReturnParcel(context, parcelId, note){
  //   dashboardLoader.value = true;
  //   update();
  //   Map<String, String> body = {'parcel_id': parcelId, 'note': note};
  //   String jsonbody = jsonEncode(body);
  //   print(body);
  //   try {
  //     server.postRequestWithToken(endPoint: APIList.spotManagerReturnUrl, body: jsonbody).then((response) {
  //       print(response.statusCode);
  //       if (response != null && response.statusCode == 200) {
  //         onInit();
  //         dashboardLoader.value = false;
  ...
  //         Get.rawSnackbar(snackPosition: SnackPosition.TOP, title: 'Change Status', message: 'returned Successfully', backgroundColor: CupertinoColors.activeGreen.withOpacity(.9), margin: const EdgeInsets.only(bottom: 20, left: 20, right: 20));
  //       } else {
  //         dashboardLoader.value = false;
  //         update();
  //       }
  ...
  changeStatusForPendingDelivery(context, statusID, parcelId) {
    dashboardLoader.value = true;

    update();
    Map<String, String> body = {'parcel_id': parcelId, 'status_action': pendingDeliveryStatusID, 'note': noteController.text};
    String jsonbody = jsonEncode(body);
    print(body);
    try {
      server.postRequestWithToken(endPoint: APIList.spotManagerChangeStatus, body: jsonbody).then((response) {
  ...
  changeStatusForReturnDelivery(context, statusID, parcelId) {
    dashboardLoader.value = true;

    update();
    Map<String, String> body = {'parcel_id': parcelId, 'status_action': returnDeliveryStatusID, 'note': noteController.text};
    String jsonbody = jsonEncode(body);
    print(body);
    try {
      server.postRequestWithToken(endPoint: APIList.spotManagerChangeStatus, body: jsonbody).then((response) {
  ...
  getDashboard() async {
    ...
    if (response.statusCode == 200) {
      ...
      returnedParcel.value = dashboard.data!.returnParcel!;
  ...
  changeStatus(context, parcelId) {
    dashboardLoader.value = true;
    update();
    final queryParameters = {'parcel_id': parcelId, 'status_action': statusID, if (statusID != '44') 'documents': avatar.value?.path, 'note': noteController.text, if (statusID != '44') 'pickup_code': pickUpCodeController.value.text};
    ...
    try {
      server.postRequestWithToken(endPoint: APIList.spotManagerChangeStatus, body: jsonString).then((response) {
        ...
        if (json['success'] == true && response.statusCode == 200) {
          ...
          Get.rawSnackbar(snackPosition: SnackPosition.TOP, title: 'Change Status', message: json['message'], backgroundColor: CupertinoColors.activeGreen.withOpacity(.9), margin: const EdgeInsets.only(bottom: 20, left: 20, right: 20));
          ...
          dashboardLoader.value = false;
          update();
          Get.rawSnackbar(title: 'Error', message: json['message'], backgroundColor: CupertinoColors.destructiveRed.withOpacity(.9), margin: const EdgeInsets.only(bottom: 20, left: 20, right: 20));
        }
      });
    } catch (e) {
      Get.log(e.toString());
  ...
  getBarcodeParcel(context, trackingid) {
    server.getRequest(endPoint: '${APIList.spotManagerBarcodeParcel!}$trackingid').then((response) {
      if (response != null && response.statusCode == 200) {
        dashboardLoader.value = false;
        final jsonResponse = json.decode(response.body);
        print(jsonResponse);
        if (jsonResponse['success'] == true) {
          if (jsonResponse['data']['parcel']['status'].toString() == '1' ||
              jsonResponse['data']['parcel']['status'].toString() == '2' ||
            ...
            fsshowPickUpPopUp(context, jsonResponse['data']['parcel']['id']);
          } else if (jsonResponse['data']['parcel']['status'].toString() == '42') {
            collectByDriverPopUp(context, jsonResponse['data']['parcel']['id']);
          } else if (jsonResponse['data']['parcel']['status'].toString() == '35' || jsonResponse['data']['parcel']['status'].toString() == '43') {
            showStatusPopUp(context, jsonResponse['data']['parcel']['id']);
          } else {
            Get.rawSnackbar(
  ...
  void fsshowPickUpPopUp(context, parcelID) {
    showDialog(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    statusID == '32' ? CustomFormField(controller: cashController, validatorTxt: 'please_type_your_amount'.tr) : SizedBox().marginZero,
                    const SizedBox(height: 10.0),
                        ...
                        TextButton(
                          ...
                          onPressed: () async {
                            ...
                            if (form!.validate()) {
                              ...
                              Get.back();
                              changeStatusWithOutSignature(context, collectByDriverStatusID, parcel.toString());
                            }
                          },
                        ),
                      ],
